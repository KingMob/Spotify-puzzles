...
class Solver {
	...
	private final Vertex friend = Vertex.vertexFromId(ProjectParams.FRIEND_ID);
 	...
 	// private boolean[] matched = new boolean[MAX_NUM_VERTICES];
 	private boolean[] visited = new boolean[MAX_NUM_VERTICES];
 	private int[] prev = new int[MAX_NUM_VERTICES];
 	// private boolean[] forward = new boolean[MAX_NUM_VERTICES];
	...
	private void processPath() {
		...
		while (v != source.getIdx()) {
			int u = prev[v];
			// FIXME!!!
			edges.flipFlow(Vertex.vertexFromId(u + 1000), Vertex.vertexFromId(v + 1000));
			v = u; // follow reverse path to source 
		}

		Arrays.fill(visited, false); // reset for next iteration 
		Arrays.fill(prev, -1); // reset for next iteration 
	}
	...
	public String toString(){
		...
		sb.append("L Vertex list:\n").append(LV).append("\n");
		sb.append("R Vertex list:\n").append(RV).append("\n");
...
class Edges {
	...
	// private List<Map<Vertex, Edge>> edgeInfo;
	...
	public Edges(int numVertices){
		...
		// capacity = new ArrayList<Map<Vertex, Edge>>(numVertices);
...
class ProjectParams {
	...
	private ProjectParams(int[][] teams, List<List<Integer>> ids){
		this.teams = teams;
		this.ids = ids;
		// this.ids = new ArrayList<List<Integer>>(NUM_LOCATIONS);

		// this.ids.add(0, new ArrayList<Integer>(ids.get(0)));
		// this.ids.add(1, new ArrayList<Integer>(ids.get(1)));
	}
	...
	public static ProjectParams fromStdIn() throws IOException {
		...
		int[][] teams = new int[NUM_LOCATIONS][numTeams];
		...
		for(int currTeam = 0; currTeam < numTeams; currTeam++){
			...
			teams[0][currTeam] = id1;
			teams[1][currTeam] = id2;
