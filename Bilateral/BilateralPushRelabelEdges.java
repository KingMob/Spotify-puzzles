...
public class BilateralPushRelabelEdges {
...
class Solver {
	...
	private void initialize(){
		LV = new ArrayList<Vertex>(pp.getIds(0).size());
		RV = new ArrayList<Vertex>(pp.getIds(1).size());

		setupSourceSide(pp.getIds(0));
		setupSinkSide(pp.getIds(1));
	...
	private void discharge(Vertex u){
		...
		while(u.getExcess() > 0){
			if(u.getCurrNeighbor() >= u.getNumNeighbors()){
				...
				Edge e = u.getEdge(u.getCurrNeighbor());
				Vertex v = e.getNeighbor(u);

				if(u.getHeight() == v.getHeight() + 1 && e.getResidualCapacity(u, v) > 0){
					push(u, v, e);
				}
				else{
					u.incNextNeighbor();
	...
	private List<Vertex> computeMinVertexCover(List<Vertex> side1, List<Vertex> side2){
		List<Vertex> konigSet = new ArrayList<Vertex>(side1.size() + side2.size());
	...
	private void konigDFS(List<Vertex> konigSet, Vertex v, boolean edgesInMatch){
...
class Vertex {
	...
	//public Vertex getNeighbor(int i){ return edges.get(i).getNeighbor(this);}
